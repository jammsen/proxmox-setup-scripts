...
echo "deb [arch=amd64] https://repo.radeon.com/rocm/apt/6.2.4 jammy main" | sudo tee /etc/apt/sources.list.d/rocm.list

apt update
apt install -y rocm-dev rocm-libs rocm-utils --no-install-recommends

# Install AMD Container Toolkit for ROCm support
curl -fsSL https://rocm.docs.amd.com/rocm.gpg.key | sudo apt-key add -
echo "deb [arch=amd64] https://rocm.docs.amd.com/apt/6.2.4 jammy main" | sudo tee -a /etc/apt/sources.list.d/rocm.list
apt update
